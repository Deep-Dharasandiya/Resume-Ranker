...
class Certificates extends StatefulWidget {
  Certificates(this.email);
  String email;
  @override
  _Certificates createState() => _Certificates(email);
}
...
class _Certificates extends State<Certificates> {
  ...
  String base64Imagecer;
  Future<File> filecer;
  File tmpFilecer;
  String fileNamecer=" ";
  choose_certificate() {
    setState(() {filecer = ImagePicker.pickImage(source: ImageSource.gallery);});
  }

  Widget show_certificate() {
    return FutureBuilder<File>(
      future: filecer,
      builder: (BuildContext context, AsyncSnapshot<File> snapshot) {
        if (snapshot.connectionState == ConnectionState.done &&
            null != snapshot.data) {
          tmpFilecer = snapshot.data;
          base64Imagecer = base64Encode(snapshot.data.readAsBytesSync());
          String filenamecer = tmpFilecer.path.split('/').last;
          var file = File(tmpFilecer.path);
          int size=file.lengthSync() ;
          if(size<=102400){
              fileNamecer=filenamecer;
            return Text(fileNamecer, textAlign: TextAlign.center,);}
          else{
            base64Imagecer=null;
            file=null;
            tmpFilecer=null;
  ...
  void insert_certificates() async {
    setState(() {
      Mainloader=true;
    });
    if(completed==false){
    ...
    final response = await http.post(
        "http://resumeranker.hopto.org/insert_resume_certificates.php",
        body: {
          "A":widget.email,
  ...
  void deletecertificates(String d)async{
    setState(() {
      Mainloader=true;
    });
    final response = await http.post(
        "http://resumeranker.hopto.org/delete_resume_certificates.php",
        body: {
          "A": d,
        }
    );
     ...
     child: Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: <Widget>[
            Expanded(
              child:Text("Certificates",style: TextStyle(color: Colors.black, fontFamily: 'design.graffiti.comicsansms'),),
            ),
        ...
        child: home==false?Column(
                  ...
                  child: Column(
                    children: <Widget>[
                                      ...
                                      children: <Widget>[
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: <Widget>[
                                              Text(" Certificates:",style: TextStyle(color: Colors.black54, fontFamily: 'design.graffiti.comicsansms'),),
                                              show_certificate(),
                                            ],
                                          ),
                                        ),
                      ...
                      RaisedButton(
                        ...
                        onPressed:(){
                          if(name2=='' || desc2=='' || completed==null || startdate==' ' ||(enddate==' ' && completed == true ) ||base64Imagecer==null ||fileNamecer==' '){
          ...
          children: <Widget>[
            Container(
              child:Stack(
                children: <Widget>[
                 certificates.length!=0? ListView.builder(
                                ...
                                children: <Widget>[
                                  Row(
                                    children: <Widget>[
                                      ...
                                      Expanded(
                                        child: Text(" "+certificates[index]['name'],style: new TextStyle(fontSize:15,color: Colors.black),),
                                      ),
                                    ],
                                  ),
                                  Row(
                                    children: <Widget>[
                                      ...
                                      Expanded(
                                        child:Text(" "+certificates[index]['aboutcourse'] ,style: new TextStyle(fontSize:15,color: Colors.black),),
                                      ),
                                    ],
                                  ),
                                  certificates[index]['completed']=='true'? Row(
                                    children: <Widget>[
                                      ...
                                      Expanded(
                                        child:Text(" "+certificates[index]['startdate']+"  To  "+certificates[index]['enddate'],style: new TextStyle(fontSize:15,color: Colors.black),),
                                      ),
                                    ],
                                  ): Row(
                                    children: <Widget>[
                                      new Icon(Icons.date_range , size: 15,color: Colors.blue,),
                                      Expanded(
                                        child:Text(" "+certificates[index]['startdate']+"  To  Running..",style: new TextStyle(fontSize:15,color: Colors.black),),
                                      ),
                                    ],
                                  ),
                                  Row(
                                    children: <Widget>[
                                      Expanded(
                                        child:  FlatButton(
                                          onPressed: () {
                                            setState(() {
                                            ID=certificates[index]['id'].toString();
                                            if(certificates[index]['completed']=='true'){
                                            completed=true;
                                            }
                                            else{
                                            completed=false;
                                            }
                                            name=TextEditingController(text:certificates[index]['name']);
                                            desc=TextEditingController(text:certificates[index]['aboutcourse']);
                                            startdate=certificates[index]['startdate'];
                                            enddate=certificates[index]['enddate'];
                                            fileNamecer=certificates[index]['certificate_name'];
                                            base64Imagecer=certificates[index]['certificate'];
                                            home=false;
                                                  ...
                                                  return AlertDialog(
                                                    ...
                                                    actions: <Widget>[
                                                      new FlatButton(
                                                        child: new Text('Yes'),
                                                        onPressed: () {
                                                          deletecertificates(certificates[index]['id'].toString());
